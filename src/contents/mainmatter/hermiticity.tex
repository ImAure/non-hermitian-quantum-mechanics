\chapter{Hermitian Operators in Quantum Mechanics}\label{ch:hermiticity}
    ...
    \section{Hermitian operators and their properties}\label{s:hermitian-operators}
        ...
        \subsection{Hermitian operators and Symmetry transformations}\label{ss:unitary-transformations}
            ...
            \end{equation}
            if we again let $\epsilon \to 0$. We can then deduce that any observable that commutes with \hB\ can be seen as the generator of a unitary transformation that leaves $\expval*{\hB}$ unchanged. Moreover, for any pair of operators \hA\ and \hB\ such that $\comm*{\hA}{\hB} = 0$, we can say that each of them is the generator of a tranformation that leaves the expectation value of the other unchanged. The relationship \eqref{eq:infinitesimal-transformation-QM} is strikingly similar to the classical relation\footnote{The convention $\acomm{f}{g} = \pdv{f}{x}\pdv{g}{p} - \pdv{f}{p}\pdv{g}{x}$ is implied.} \cite{Goldstein2005-ep}
            \begin{equation}
